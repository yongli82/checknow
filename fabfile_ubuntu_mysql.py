...
env.proj_app = real_project_name("checknow")
...
env.db_root_pass = conf.get("DB_ROOT_PASS", None)
...
def mysql(command):
    ...
    #return sudo(command, show=show, user="root")


@task
def mysql_command(sql, show=True):
    """
    Runs SQL against the project's database.
    """
    out = mysql('mysql -u%s -p%s -e "%s"' % ('root', env.db_root_pass, sql))
    if show:
        print_command(sql)
    return out
...
def backup(filename):
    ...
    # elsewhere.
    with cd("/"):
        mysql("mysqldump -u%s -p%s %s > %s" % (env.db_user, env.db_pass, env.db_name, tmp_file))
    run("cp %s ." % tmp_file)
...
def restore(filename):
    ...
    """
    return mysql('mysql -u%s -p%s %s < "%s"' % (env.db_user, env.db_pass, env.db_name, filename))
...
def install():
    ...
    apt("nginx libjpeg-dev python-dev python-setuptools git-core "
        "mysql-server mysql-client libmysqlclient-dev libpq-dev memcached supervisor python-pip"
        "libncurses5-dev libjpeg8 libjpeg8-dev")
...
def create():
    ...
    #pw = db_pass()
    #user_sql_args = (env.db_user, env.db_pass)
    #user_sql = "CREATE USER %s WITH ENCRYPTED PASSWORD '%s';" % user_sql_args
    #mysql_command(user_sql, show=False)
    #shadowed = "*" * len(pw)
    #print_command(user_sql.replace("'%s'" % pw, "'%s'" % shadowed))
    mysql_command("DROP DATABASE IF EXISTS %s;" % env.db_name)
    #mysql_command("DROP USER %s';" % env.db_user)
    mysql_command("create database %s DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;" % env.db_name)
    mysql_command("CREATE USER '%s'@'%%' IDENTIFIED BY '%s';" % (env.db_user, env.db_pass))
    mysql_command("GRANT ALL PRIVILEGES ON %s.* TO '%s'@'%%';FLUSH PRIVILEGES;" % (env.db_name, env.db_user))
    ...
    with project():
        if env.reqs_path:
            pip("-r %s/%s" % (env.proj_path, env.reqs_path))
        pip("gunicorn setproctitle MySQL-python "
...
def remove():
    ...
    mysql_command("DROP DATABASE IF EXISTS %s;" % env.db_name)
    mysql_command("DROP USER IF EXISTS %s;" % env.db_user)
